...
statistics_df = statistics_df[statistics_df["Task ID"].isin(tasks)]
...
raw_df = raw_df[raw_df["Task Type"] == "regression"]
raw_df["Task ID"] = raw_df["Task ID"].astype(str)
print(raw_df.shape)

fig, axs = plt.subplots(nrows=len(tasks), ncols=1, figsize=(8, 16))
df = statistics_df
...
for i, task in enumerate(tasks):
    ...
    axs[i].set_ylabel("RMSE", fontsize=12)
...
plt.tight_layout()
plt.savefig(output_dir / "regression.png", dpi=300)